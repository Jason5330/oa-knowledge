...
const desktop=path.resolve(__dirname,'..'),root=path.dirname(desktop),output=path.join(desktop,'release/OA-Knowledge-0.3.2-win-x64');
...
const app=path.join(output,'resources/app'),dest=path.join(app,'desktop');fs.mkdirSync(dest,{recursive:true});fs.writeFileSync(path.join(app,'package.json'),JSON.stringify({name:'oa-knowledge',version:'0.3.2',main:'desktop/main.cjs',private:true}));
