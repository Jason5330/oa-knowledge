...
class Store{
  ...
  listDocuments(workspace,{offset=0,limit=100,filter=''}={}){this.workspace(workspace);integer(offset,0,1e7);integer(limit,1,1000);if(typeof filter!=='string'||filter.length>200)throw new Error('篩選文字過長');return this.db.prepare('SELECT d.id,d.name AS title,d.created,length(d.original) AS bytes,length(d.text) AS characters,COUNT(c.id) AS chunks,json_extract(d.pages,\'$[0].workbook.parserVersion\') AS excelParserVersion,json_array_length(d.pages,\'$[0].workbook.tables\') AS excelTableCount FROM documents d LEFT JOIN chunks c ON c.document=d.id WHERE d.workspace=? AND instr(lower(d.name),lower(?))>0 GROUP BY d.id ORDER BY d.created,d.id LIMIT ? OFFSET ?').all(workspace,filter,limit,offset).map(({excelParserVersion,excelTableCount,...d})=>({...d,structured:require('./excel-info.cjs').info(excelParserVersion,excelTableCount)}));}
  ...
  readDocument({workspace,documentId,offset=0,length=4000}){integer(offset,0,1e8);integer(length,1,8000);const d=this.document(workspace,documentId);const book=JSON.parse(d.pages)[0]?.workbook;return {documentId,title:d.name,structured:require('./excel-info.cjs').info(book?.parserVersion,book?.tables?.length),offset,totalCharacters:d.text.length,text:d.text.slice(offset,offset+length),nextOffset:offset+length<d.text.length?offset+length:null};}
